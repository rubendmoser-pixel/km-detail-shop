...
test("production master imports every validated recipe idempotently by KM code and EAN", async (t) => {
  ...
  assert.equal(getCommercialSettings(db).profitMarginMinimumBps, 0);
  assert.equal(getCommercialSettings(db).profitMarginMediumBps, 0);
  assert.equal(getCommercialSettings(db).profitMarginMaximumBps, 0);
  updateCommercialSettings(db, { usdExchangeRate: 1525.5, productionHourlyCostArs: 4850.75, maximumDiscountBps: 2750, maximumCommissionBps: 1225, profitMarginMinimumBps: 1500, profitMarginMediumBps: 2500, profitMarginMaximumBps: 3500 }, admin.id);
  ...
  assert.equal(getCommercialSettings(db).profitMarginMinimumBps, 1500);
  assert.equal(getCommercialSettings(db).profitMarginMediumBps, 2500);
  assert.equal(getCommercialSettings(db).profitMarginMaximumBps, 3500);
  ...
  assert.throws(() => updateCommercialSettings(db, { profitMarginMinimumBps: 3000 }, admin.id), /orden/i);
  assert.throws(() => updateCommercialSettings(db, { profitMarginMaximumBps: 10001 }, admin.id), /margen máximo/i);
