...
test("HTTP API supports the initial B2B purchase flow", async (t) => {
  ...
  assert.equal(publicProductPageResponse.status, 410);
  const publicProductPage = await publicProductPageResponse.text();
  assert.equal(publicProductPageResponse.headers.get("x-robots-tag"), "noindex, nofollow");
  assert.match(publicProductPage, /ficha de producto fue retirada/i);
  ...
  assert.match(productDirectory, /id="producto-api001k"/);
  assert.doesNotMatch(productDirectory, /href="\/producto\//);
  assert.doesNotMatch(productDirectory, /ficha tecnica/i);
  assert.match(productDirectory, /"@type":"ItemList"/);
  const sitemapResponse = await fetch(`${baseUrl}/sitemap.xml`);
  assert.equal(sitemapResponse.status, 200);
  const sitemap = await sitemapResponse.text();
  assert.doesNotMatch(sitemap, /\/producto\//);
  assert.match(sitemap, /<loc>https:\/\/www\.km-detail\.com\/productos<\/loc>/);
  ...
  const dashboardAfterOrder = await getJson(`${baseUrl}/api/admin/operation/dashboard`, adminCookie);
  assert.equal(dashboardAfterOrder.dashboard.summary.totalOrders, 1);
  assert.equal(dashboardAfterOrder.dashboard.summary.activeOrders, 1);
  assert.equal(dashboardAfterOrder.dashboard.queues.received, 1);
  assert.equal(dashboardAfterOrder.dashboard.sales.bySalesRep[0].email, "vendedor-api@km-detail.com");
  assert.equal(dashboardAfterOrder.dashboard.products.some((item) => item.kmCode === "API001K"), true);
