...
export function createEmailService({ db, config, pushService = null }) {
  ...
  function queueSalesQuoteCustomer(quote) {
    ...
    queue("sales_quote_customer", quote.customerEmail, `Presupuesto ${quote.quoteNumber || ""} | KM Detail Line`, [
      ...
      quote.kind === "prospect" ? `Total a precio de lista: ${money.format((quote.subtotalListCents || 0) / 100)}` : "",
      discountLine ? `Descuento activo: ${discountLine}` : "",
      quote.kind === "prospect" ? `Ahorro por descuento: ${money.format((quote.discountCents || 0) / 100)}` : "",
